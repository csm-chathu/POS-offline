  ...
  return (
    ...
    <div style={{ ...zoomStyle, ...(theme === 'dark' ? { backgroundColor: '#1c1c1c' } : {}) }} className="flex h-screen bg-slate-200 overflow-hidden">
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
        ...
        {/* Route loading bar */}
        <div className={`h-0.5 w-full shrink-0 transition-opacity duration-200 print:hidden ${isPOS ? 'hidden' : routing ? 'opacity-100' : 'opacity-0'}`}>
        ...
        {!hideHeader && (
          <header style={theme === 'dark' ? { backgroundColor: sidebarBg, borderColor: '#2a2a2a' } : {}} className="print:hidden h-14 bg-white border-b border-slate-200 flex items-center justify-between px-3 md:px-6 shrink-0 shadow-sm">
            ...
            <div className="flex items-center gap-1.5 md:gap-3">
              {/* Help */}
              <NavLink to="/help" title="Help & User Guide"
                className={({ isActive }) => `p-2 rounded-xl transition-colors ${isActive ? 'text-orange-500 bg-orange-50 dark:bg-orange-500/10' : 'text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:hover:text-slate-200 hover:bg-slate-100 dark:hover:bg-[#252525]'}`}>
                <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
